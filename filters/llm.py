...
ВАЖНО: пользователю нужны ТОЛЬКО фриланс-заказы — разовые проекты
(заказ сделать сайт/скрипт/бота) либо долгосрочное сотрудничество с
исполнителем. Объявления о найме сотрудника на работу (вакансии,
собеседования, "по результатам собеседования", полная занятость, оклад,
"требуется программист на работу", "в штат") НЕ релевантны — отвечай
RELEVANT: NO. Ключевой признак вакансии: нанимают исполнителя в штат/на
постоянную работу, а не заказывают разовую услугу.

Темы пользователя узкие — только веб-фронтенд, вёрстка сайтов (HTML/CSS,
лендинги), парсинг и сбор данных, скрипты и автоматизация, чат-боты и
Telegram-боты. НЕ относятся к темам и должны получать RELEVANT: NO:
разработка нативных мобильных приложений (iOS/Android), десктоп-приложения,
игры, ML/AI-исследования, DevOps/администрирование, чистый дизайн/UI-макеты
без вёрстки, копирайтинг. Слово "верстка" в контексте приложений
("верстка книжная", "книжная ориентация") — это раскладка экрана, а НЕ
вёрстка сайтов, поэтому НЕ относится к теме markup."""
